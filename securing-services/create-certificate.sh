...
openssl req -config $COURSEDIR/securing-services/openssl.cnf -key ca.key.pem -new -x509 -days 7300 -sha256 -extensions v3_ca -passin 'pass:konglabs' -subj "/C=WD/ST=Earth/L=Global/O=Kong Inc./CN=Kong CA" -out ca.cert.pem
...
openssl ca -batch -config $COURSEDIR/securing-services/openssl.cnf -extensions usr_cert -cert ca.cert.pem -keyfile ca.key.pem -passin 'pass:konglabs' -in client.csr -out client.crt
